...
public class TelaAgenda extends JFrame {
	...
	public TelaAgenda() {
		...
		btnEditar.setBounds(393, 147, 97, 25);
		panel.add(btnEditar);
		...
		listId.addListSelectionListener(new ListSelectionListener() {
			public void valueChanged(ListSelectionEvent arg0) {
				BancoDeDados banco = new BancoDeDados();
				banco.conectar();
				banco.buscarAgenda(listId, txtData, txtTermino, txtInicio, comboVet, comboCliente, comboPet, comboTipo, txtRecomendacao, txtDescricao, idAgenda);
				banco.desconectar();
			}
		});
